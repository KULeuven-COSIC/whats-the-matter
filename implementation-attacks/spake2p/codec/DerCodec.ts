...
/**
 * Used for encoding and decoding data into the DER format, which ensures that data structures are represented 
 * in a consistent and compact binary form, suitable for cryptographic applications, digital certificates, etc.
 */
...
export class DerCodec {
        ...
        if (Array.isArray(value)) {
            ...
            if (value[TAG_ID_KEY] !== undefined) {
                ...
                if (typeof tagId !== "number") {
                    throw new Error("Tag ID is non-numeric");
                }
                if (bitsPadding !== undefined && typeof bitsPadding !== "number") {
                    throw new Error("Bits padding is not a numeric byte value");
                }
                if (bytes === undefined || !ArrayBuffer.isView(bytes)) {
                    throw new Error("DER bytes is not a byte array");
                }
                ...
                if (value[TYPE_OVERRIDE_KEY] === DerType.Integer && value[RAW_DATA_KEY] instanceof ByteArray) {
                    ...
                    throw new Error(`Unsupported override type ${value[TYPE_OVERRIDE_KEY]}`);
                }
            } else if (
    ...
    private static encodePrintableString(value: string) {
        if (!/^[A-Za-z0-9 '()+,-./:=?]*$/g.test(value)) {
            throw new Error(`String ${value} is not a printable string.`);
        }
    ...
    private static encodeIA5String(value: string) {
        ...
        if (!/^[\x00-\x7F]*$/.test(value)) {
            throw new Error(`String ${value} is not an IA5 string.`);
        }
