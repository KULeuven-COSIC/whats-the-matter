...
try: # try block for nice cleanup on Ctrl+C
    while MAX_CYCLES is None or cycle_num < MAX_CYCLES:
        ...
        restart_times.append(restart_duration) 
        ...
        attempt_times.append(attempts_duration)
    ...
    if restart_times:
        restart_median = np.median(restart_times)
        restart_mean = np.mean(restart_times)
        restart_std = np.std(restart_times, ddof=1)
        restart_conf_interval = 1.96 * (restart_std / np.sqrt(len(restart_times)))
        print("\n--- Restart Times Statistics ---")
        print(f"Median restart time: {restart_median:.2f} seconds")
        print(f"Mean restart time: {restart_mean:.2f} seconds")
        print(f"95% confidence interval for restart time: ±{restart_conf_interval:.2f} seconds")

    if attempt_times:
        attempt_median = np.median(attempt_times)
        attempt_mean = np.mean(attempt_times)
        attempt_std = np.std(attempt_times, ddof=1)
        attempt_conf_interval = 1.96 * (attempt_std / np.sqrt(len(attempt_times)))
        print("\n--- Attempt Times Statistics ---")
        print(f"Median attempt time: {attempt_median:.2f} seconds")
        print(f"Mean attempt time: {attempt_mean:.2f} seconds")
        print(f"95% confidence interval for attempt time: ±{attempt_conf_interval:.2f} seconds")    